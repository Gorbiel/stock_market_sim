...
public class WalletController {
    ...
    @PostMapping(value = "/wallets/{walletId}/stocks/{stockName}", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Void> processOperation(
            @Parameter(description = "Wallet ID", example = "wallet-1", required = true) @PathVariable String walletId,
            @Parameter(description = "Stock name", example = "stock1", required = true) @PathVariable String stockName,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                            required = true,
                            content = @Content(schema = @Schema(implementation = WalletStockOperationRequest.class)))
                    @Valid
                    @RequestBody
    ...
    @Operation(summary = "Get wallet state", description = "Returns all stocks currently held by the wallet.")
    @ApiResponses({
        @ApiResponse(
                responseCode = "200",
    ...
    public ResponseEntity<WalletResponse> getWallet(
            @Parameter(description = "Wallet ID", example = "wallet-1", required = true) @PathVariable
                    String walletId) {
    ...
    public ResponseEntity<Integer> getWalletStockQuantity(
            @Parameter(description = "Wallet ID", example = "wallet-1", required = true) @PathVariable String walletId,
            @Parameter(description = "Stock name", example = "stock1", required = true) @PathVariable
                    String stockName) {
